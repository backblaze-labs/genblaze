...
- `_ATTEMPT_HASH_FIELDS` — each `Step.failed_attempts` entry (#239) is reduced to `{model, provider, error_code}` before hashing: which model failed, where, and why is integrity-checked, while the attempt's `step_id`, timestamps, `error`, `cost_usd`, `retries` and `upstream_id` are operational, like their Step counterparts. An empty list is omitted from serialization entirely, so manifests without a fallback hash exactly as before the field existed. Manifests that carry `failed_attempts` still declare schema 1.5 but need a reader that knows the key; older genblaze-core releases reject it as an unknown Step field.
